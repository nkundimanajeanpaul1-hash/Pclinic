...
(function () {
    ...
    function buildToolbar() {
        ...
        main.appendChild(tb('wl', 'Window/Level', 'Window / level (drag). Right-click for presets', function () { setLeftTool('Wwwc'); }, { tool: 'Wwwc' }));
        main.appendChild(tb('pan', 'Pan', 'Pan (drag). Middle mouse always pans', function () { setLeftTool('Pan'); }, { tool: 'Pan' }));
        main.appendChild(tb('zoom', 'Zoom', 'Zoom (drag). Right mouse / pinch always zooms', function () { setLeftTool('Zoom'); }, { tool: 'Zoom' }));
        main.appendChild(tb('zoomin', '', 'Zoom in', function () { zoomBy(1.25); }));
        main.appendChild(tb('zoomout', '', 'Zoom out', function () { zoomBy(0.8); }));
        main.appendChild(tb('fit', 'Fit', 'Fit to window', fitToWindow, { iconOnly: true }));
        main.appendChild(sep());
        main.appendChild(tb('layout', 'Layout', 'Viewport layout', function (e, b) { openMenu(b, layoutMenu()); }, { menu: true }));
        main.appendChild(tb('sync', 'Sync', 'Synchronise zoom/pan and window/level between viewports', toggleSync, { id: 'sync', iconOnly: true }));
        main.appendChild(tb('reset', 'Reset', 'Reset view (zoom, pan, W/L, rotation)', resetView, { iconOnly: true }));
        main.appendChild(sep());
        main.appendChild(tb('ruler', 'Ruler', 'Length measurement', function () { setLeftTool('Length'); }, { tool: 'Length' }));
        main.appendChild(tb('angle', 'Angle', 'Angle measurement', function () { setLeftTool('Angle'); }, { tool: 'Angle', mid: true }));
        main.appendChild(tb('roi', 'ROI', 'Region of interest: rectangle, ellipse, freehand, bidirectional, Cobb', function (e, b) { openMenu(b, roiMenu()); }, { menu: true, id: 'roi' }));
        main.appendChild(tb('probe', 'Probe', 'Pixel value probe', function () { setLeftTool('Probe'); }, { tool: 'Probe', iconOnly: true }));
        main.appendChild(tb('arrow', 'Arrow', 'Arrow with text annotation', function () { setLeftTool('ArrowAnnotate'); }, { tool: 'ArrowAnnotate', mid: true }));
        main.appendChild(tb('trash', 'Delete', 'Delete measurements', function (e, b) { openMenu(b, deleteMenu()); }, { menu: true, mid: true }));
        main.appendChild(tb('star', 'Key image', 'Mark / unmark this image as a key image (saved to the common server, K)', function () { toggleKeyImage(); }, { id: 'key', mid: true }));
        main.appendChild(sep());
        main.appendChild(tb('magnify', 'Magnify', 'Magnifying glass (drag)', function () { setLeftTool('Magnify'); }, { tool: 'Magnify', iconOnly: true }));
        main.appendChild(tb('rotate', 'Rotate', 'Rotate / flip', function (e, b) { openMenu(b, rotateMenu()); }, { menu: true }));
        main.appendChild(tb('flip', 'Flip', 'Flip horizontally', function () { flip('h'); }, { iconOnly: true }));
        main.appendChild(tb('invert', 'Invert', 'Invert grey scale', invert, { id: 'invert', iconOnly: true }));
        main.appendChild(tb('presets', 'Presets', 'Window / level presets and colour maps', function (e, b) { openMenu(b, presetMenu()); }, { menu: true }));
        main.appendChild(sep());
        main.appendChild(tb('cine', 'Cine', 'Cine loop (multi-frame / series)', toggleCine, { id: 'cine' }));
        main.appendChild(tb('prev', '', 'Previous frame / image (←)', function () { step(-1); }));
