...
describe('reception cross-role integrations', () => {
  ...
  test('common-server staff messages support direct recipient, sender visibility, reply, and role broadcast', async () => {
    await assertSucceeds(setDoc(doc(dbFor('reception'), 'messages', 'msg-direct-1'), {
      id:'msg-direct-1', text:'Please review the patient at reception.',
      toRoles:[], toStaffId:profiles.doctor.staffId, toStaffName:profiles.doctor.name, toRole:'doctor', audience:'staff',
      priority:'normal', patientId:'1001', patientName:'Demo Patient', category:'message',
      fromName:profiles.reception.name, fromId:profiles.reception.staffId, fromRole:'reception',
      threadId:'thread-1', replyToId:'', at:'2026-09-09T08:00:00.000Z', readBy:[]
    }));
    await assertSucceeds(getDoc(doc(dbFor('doctor'), 'messages', 'msg-direct-1')));
    await assertSucceeds(getDoc(doc(dbFor('reception'), 'messages', 'msg-direct-1')));
    await assertFails(getDoc(doc(dbFor('nurse'), 'messages', 'msg-direct-1')));

    await assertSucceeds(setDoc(doc(dbFor('doctor'), 'messages', 'msg-reply-1'), {
      id:'msg-reply-1', text:'I will review the patient now.',
      toRoles:[], toStaffId:profiles.reception.staffId, toStaffName:profiles.reception.name, toRole:'reception', audience:'staff',
      priority:'normal', patientId:'1001', patientName:'Demo Patient', category:'message',
      fromName:profiles.doctor.name, fromId:profiles.doctor.staffId, fromRole:'doctor',
      threadId:'thread-1', replyToId:'msg-direct-1', at:'2026-09-09T08:05:00.000Z', readBy:[]
    }));
    await assertSucceeds(getDoc(doc(dbFor('reception'), 'messages', 'msg-reply-1')));
    await assertSucceeds(getDoc(doc(dbFor('doctor'), 'messages', 'msg-reply-1')));
    await assertFails(getDoc(doc(dbFor('nurse'), 'messages', 'msg-reply-1')));

    await assertSucceeds(setDoc(doc(dbFor('reception'), 'messages', 'msg-broadcast-1'), {
      id:'msg-broadcast-1', text:'All nurses please prepare for the incoming transfer.',
      toRoles:['nurse'], toStaffId:'', toStaffName:'', toRole:'', audience:'role',
      priority:'urgent', patientId:'1001', patientName:'Demo Patient', category:'message',
      fromName:profiles.reception.name, fromId:profiles.reception.staffId, fromRole:'reception',
      threadId:'thread-2', replyToId:'', at:'2026-09-09T08:10:00.000Z', readBy:[]
    }));
    await assertSucceeds(getDoc(doc(dbFor('nurse'), 'messages', 'msg-broadcast-1')));
    await assertSucceeds(getDoc(doc(dbFor('reception'), 'messages', 'msg-broadcast-1')));
    await assertFails(getDoc(doc(dbFor('doctor'), 'messages', 'msg-broadcast-1')));
  });
});
